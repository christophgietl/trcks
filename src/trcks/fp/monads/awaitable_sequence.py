"""Monadic functions for [collections.abc.Awaitable][] of [collections.abc.Sequence][].

Provides utilities for functional composition of asynchronous functions
that work on sequences and/or return sequences.
...
def construct(value: _T) -> Awaitable[Sequence[_T]]:
    """Create an awaitable sequence from a single value.

    Args:
        value: A single value.

    Returns:
        An [collections.abc.Awaitable][] that produces a
        [collections.abc.Sequence][] containing the single value.
...
def construct_from_awaitable(awtbl: Awaitable[_T]) -> Awaitable[Sequence[_T]]:
    """Create an awaitable sequence from an awaitable value.

    Args:
        awtbl: Awaitable that produces a single value.

    Returns:
        Awaitable that produces a single-element sequence of the value.
...
def construct_from_sequence(seq: Sequence[_T]) -> Awaitable[Sequence[_T]]:
    """Wrap a sequence in an awaitable.

    Args:
        seq: The sequence to wrap.

    Returns:
        An awaitable that yields the given sequence.
...
def map_(
    f: Callable[[_T1], _T2],
) -> Callable[[Awaitable[Sequence[_T1]]], Awaitable[Sequence[_T2]]]:
    """Map a synchronous function over each element of an awaitable sequence.

    Args:
        f: Function to apply to each element.

    Returns:
        Function that takes an awaitable sequence and yields an awaitable
        sequence with `f` applied element-wise.
...
def map_to_awaitable(
    ...
    """Map an awaitable-returning function over an awaitable sequence.
    ...
    Returns:
        Function that takes an awaitable sequence and returns an awaitable
        sequence of the transformed values.
...
def map_to_awaitable_sequence(
    f: Callable[[_T1], Awaitable[Sequence[_T2]]],
) -> Callable[[Awaitable[Sequence[_T1]]], Awaitable[Sequence[_T2]]]:
    """Map an awaitable sequence-returning function and flatten the results.

    Args:
        f: Asynchronous function returning a sequence for each element.

    Returns:
        Function that takes an awaitable sequence, applies `f` to each element,
        awaits and flattens the resulting sequences.
    ...
    async def mapped_f(a_t1s: Awaitable[Sequence[_T1]]) -> Sequence[_T2]:
...
def map_to_sequence(
    ...
    """Map a sequence-returning function over an awaitable sequence and flatten.
    ...
    Returns:
        Function that takes an awaitable sequence, applies `f` to each element,
        and returns an awaitable flattened sequence.
...
) -> Callable[[Awaitable[Sequence[_T1]]], Awaitable[Sequence[_T1]]]:
    """Apply a side effect to each element, passing through the original sequence.

    Args:
        f: Side-effect function to apply to each element.

    Returns:
        Function that takes an awaitable sequence, applies the side effect,
...
def tap_to_awaitable(
    ...
    """Apply an awaitable side effect to each element, passing through values.
...
def tap_to_awaitable_sequence(
    f: Callable[[_T1], Awaitable[Sequence[object]]],
) -> Callable[[Awaitable[Sequence[_T1]]], Awaitable[Sequence[_T1]]]:
    """Apply an awaitable sequence-returning side effect to each element.
    ...
    Returns:
        Function that takes an awaitable sequence, applies the side effect,
        and returns a sequence of the original elements, replicated accordingly.
...
async def to_coroutine_sequence(a_seq: Awaitable[Sequence[_T]]) -> Sequence[_T]:
    """Turn an awaitable sequence into a coroutine that yields a sequence.

    Useful for functions expecting a coroutine (e.g., [asyncio.run][]).

    Args:
        a_seq: Awaitable that yields a sequence.

    Returns:
        The resulting sequence after awaiting.
