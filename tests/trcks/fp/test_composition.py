...
def test_compose_correctly_composes_8_functions() -> None:
    composed = compose(*_EIGHT_INCRS)
    assert_type(composed, Callable[[int], int])
    assert composed(0) == len(_EIGHT_INCRS)


def test_compose_correctly_composes_9_functions() -> None:
    composed = compose(*_NINE_INCRS)
    assert_type(composed, Callable[[int], int])
    assert composed(0) == len(_NINE_INCRS)
...
def test_pipe_correctly_applies_8_functions() -> None:
    piped = pipe(0, *_EIGHT_INCRS)
    assert_type(piped, int)
    assert piped == len(_EIGHT_INCRS)


def test_pipe_correctly_applies_9_functions() -> None:
    piped = pipe(0, *_NINE_INCRS)
    assert_type(piped, int)
    assert piped == len(_NINE_INCRS)
